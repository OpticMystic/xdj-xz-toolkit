...
int main(void) {
    ...
    for(int deck=0;deck<2;deck++) {m.deck[deck].ready=XZ_UI_STEM;m.deck[deck].levels[0]=1;}
    ...
    assert(xz_ui_inline_touch(&u,&m,536,60,120,10,1,a)==1);
    assert(a[0].kind==XZ_UI_MUTE&&a[0].index==0&&a[0].deck==0&&a[0].phase==XZ_UI_PRESS);
    u.deck=1;
    assert(xz_ui_inline_touch(&u,&m,536,60,-10,-10,0,a)==1);
    assert(a[0].kind==XZ_UI_MUTE&&a[0].deck==0&&a[0].phase==XZ_UI_RELEASE);
    assert(xz_ui_inline_touch(&u,&m,536,60,120,40,1,a)==1);
    ...
    xz_ui_inline_touch(&u,&m,536,60,900,40,0,a);
