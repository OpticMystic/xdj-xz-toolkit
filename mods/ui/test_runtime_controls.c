...
int xz_audio_waveform(int deck,unsigned role,unsigned char *bins,size_t count,float *progress){(void)deck;(void)role;memset(bins,16,count);if(progress)*progress=0.5;return 1;}
...
int main(void){
    ...
    contact(self,270,232,1);contact(self,270,232,0);assert(model.deck[0].muted==4&&applied[0].vocals==0&&stock_calls==0);
    contact(self,270,232,1);contact(self,270,232,0);assert(!model.deck[0].muted&&applied[0].vocals==1);
    contact(self,141,232,1);contact(self,141,232,0);assert(inline_ui.deck==1&&ui.deck==1);
    contact(self,270,270,1);int calls=stock_calls;native_active=0;contact(self,799,400,1);contact(self,799,400,0);assert(stock_calls==calls&&!inline_contact);
    native_active=1;contact(self,30,400,1);contact(self,270,232,1);contact(self,270,232,0);assert(stock_calls==calls+3);
    contact(self,200,80,1);assert(last_stock.x==200&&last_stock.y==18+xz_wave_source_row(62,100));contact(self,200,250,1);assert(last_stock.y==314);contact(self,200,250,0);
    contact(self,270,270,1);fixture_generation++;contact(self,650,270,1);assert(model.deck[1].levels[2]==1);contact(self,650,270,0);
