...
int main(void) {
    for(unsigned y=0;y<10;y++) {
        ...
        assert(xz_wave_source_row(y+132,100)==y+136);
        assert(xz_wave_source_row(y+222,100)==y+258);
    }
    assert(xz_wave_source_row(132,100)==136);
    assert(xz_wave_source_row(231,100)==267);
    assert(xz_wave_source_row(999,100)==267);
    for (unsigned height=21;height<=102;height++) {
        native_redraw();
        assert(xz_wave_compact(pixels,540*268,540,height));
        for (unsigned y=0;y<268;y++) for (unsigned x=0;x<540;x++) {
            int waveform=y<height||(y>=height+32&&y<height*2+32);
    ...
    assert(!xz_wave_compact(pixels,540*268,540,103));
