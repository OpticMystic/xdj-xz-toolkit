...
unsigned xz_wave_source_row(unsigned row,unsigned lane_height) {
    ...
    if(row<lane_height+32)return 132;
    if(row<lane_height*2+32)return 136+lane_row(row-lane_height-32,lane_height);
    return 267;
}
int xz_wave_compact(uint16_t *pixels,size_t count,size_t stride,unsigned lane_height) {
    if(!pixels||lane_height<=20||lane_height>102||stride<536||
    ...
    for(unsigned y=lane_height+32;y<lane_height*2+32;y++) {
        unsigned source=xz_wave_source_row(y,lane_height);
        memmove(pixels+(size_t)y*stride,pixels+(size_t)source*stride,536*sizeof(*pixels));
    }
    for(unsigned y=lane_height;y<lane_height+32;y++)memset(pixels+(size_t)y*stride,0,536*sizeof(*pixels));
    for(unsigned y=lane_height*2+32;y<268;y++)memset(pixels+(size_t)y*stride,0,536*sizeof(*pixels));
    return 1;
}
