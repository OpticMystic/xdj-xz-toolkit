...
static pthread_once_t mods_bridge_once = PTHREAD_ONCE_INIT;
...
static pthread_mutex_t mods_status_mutex = PTHREAD_MUTEX_INITIALIZER;
...
/* Never draw mod controls into composite_frame: it persists between packets. */
static uint16_t mods_present_frame[VJFS_MAX_WIDTH * VJFS_MAX_HEIGHT];
...
static void resolve_mods_bridge(void) {
...
static int mods_panel_visible(void) {
    return mods_visible && mods_visible();
}

static int mods_owns_native_touch(void) {
    return mods_native_touch && mods_native_touch();
}
...
#ifdef XZ_MODS_BRIDGE_V1
        /* The native solver supplies calibrated edges through xz_vj_touch_v1.
         * Do not also emit the guessed raw coordinates below. */
        if (mods_owns_native_touch()) return result;
#endif
        const uint8_t *bytes = (const uint8_t *)buffer;
...
static int overlay_is_active(void) {
    return mods_panel_visible() || vj_overlay_is_active();
}

static void connection_snapshot(struct xz_vj_connection_v1 *out) {
    static struct xz_vj_connection_v1 cached;
    static uint32_t checked_at;
    static uint32_t previous_images;
    static int sampled;
    uint32_t now = wallclock_ms();
    memset(out, 0, sizeof(*out));
    if (pthread_mutex_trylock(&mods_status_mutex) == 0) {
        uint32_t elapsed = now - checked_at;
        if (!sampled || elapsed >= 1000u) {
            uint32_t images = __atomic_load_n(&vj_received_image_count, __ATOMIC_RELAXED);
            ...
            if (fd >= 0) {
                ...
                if (ioctl(fd, SIOCGIFADDR, &request) == 0) {
                    ...
                    if (!inet_ntop(AF_INET, &address->sin_addr, cached.ipv4, sizeof(cached.ipv4)))
                        cached.ipv4[0] = '\0';
                }
                close(fd);
            }
            if (sampled && elapsed >= 1000u && elapsed <= 5000u) {
                cached.frame_hz_milli = (uint32_t)((uint64_t)(images - previous_images) * 1000000u / elapsed);
                cached.stats_valid = 1;
            } else {
                cached.frame_hz_milli = 0;
                cached.stats_valid = 0;
            }
        ...
        *out = cached;
        pthread_mutex_unlock(&mods_status_mutex);
    }
    out->version = XZ_MODS_BRIDGE_V1;
    out->size = (uint32_t)sizeof(*out);
    out->listening = __atomic_load_n(&vj_listener_ready, __ATOMIC_ACQUIRE);
    out->discovery = __atomic_load_n(&vj_discovery_ready, __ATOMIC_ACQUIRE);
    out->connected = vjfs_client_fd >= 0;
    out->active = vj_overlay_is_active() != 0;
    if (!out->connected) { out->stats_valid = 0; out->frame_hz_milli = 0; }
}

static int render_mods(uint16_t *pixels, uint32_t width, uint32_t height, uint32_t stride) {
    struct xz_vj_connection_v1 connection;
    if (!mods_render) return 0;
    connection_snapshot(&connection);
    return mods_render(pixels, width, height, stride, &connection) == 1;
}

static const uint8_t *composite_for_present(void) {
    if (mods_render) {
        memcpy(mods_present_frame, composite_frame, sizeof(composite_frame));
        if (render_mods(mods_present_frame, 800, 480, 800))
            return (const uint8_t *)mods_present_frame;
        /* A visible panel may be busy publishing its next model. Retain the
         * already-presented panel instead of flashing the desktop base. */
        if (mods_panel_visible()) return NULL;
    }
...
static void *framebuffer_presenter(void *unused) {
    ...
    for (;;) {
        ...
        if (overlay_is_active()) {
            ...
            const uint8_t *present_frame = composite_for_present();
            if (present_frame) {
                for (int row = 0; row < 480; ++row) {
                    memcpy(
                        mapping + (size_t)row * fix.line_length,
                        present_frame + (size_t)row * 1600u,
                        1600u
                    );
                }
...
static int draw_mods_on_surface(IDirectFBSurface *surface) {
    int width = 0, height = 0, pitch = 0, drawn = 0;
    void *pixels = NULL;
    DFBSurfacePixelFormat format = DSPF_UNKNOWN;
    if (!mods_render) return 0;
    if (surface->GetSize(surface, &width, &height) != DFB_OK || width < 800 || height < 480) return 0;
    if (surface->GetPixelFormat(surface, &format) != DFB_OK || format != DSPF_RGB16) return 0;
    if (surface->Lock(surface, DSLF_WRITE, &pixels, &pitch) != DFB_OK || !pixels) return 0;
    if (pitch >= width * 2 && (pitch & 1) == 0)
        drawn = render_mods((uint16_t *)pixels, (uint32_t)width, (uint32_t)height, (uint32_t)pitch / 2u);
    surface->Unlock(surface);
    return drawn;
}
...
static DFBResult hooked_primary_flip(
    ...
    /* Closed-panel badge is painted onto stock flips too. A busy/no-draw
     * renderer must leave the original partial-flip region and flags intact. */
    mods_drawn = draw_mods_on_surface(thiz);
    if (mods_render && mods_panel_visible() && !mods_drawn) return DFB_OK;
    if (directfb_interface && overlay_enabled) directfb_interface->WaitIdle(directfb_interface);
    ...
    result = original_primary_flip(
        thiz,
        (overlay_enabled || mods_drawn) ? NULL : region,
        (overlay_enabled || mods_drawn) ? DSFLIP_WAITFORSYNC : flags
    );
    flip_count += 1;
    write_runtime_stats_if_due();
    return result;
...
static void handle_vjfs_client(int client) {
    ...
    while (read_full(client, &header, sizeof(header))) {
        ...
        if (memcmp(header.magic, "VJVP", 4) == 0) {
            ...
            __atomic_fetch_add(&vj_received_image_count, 1u, __ATOMIC_RELAXED);
            pthread_mutex_unlock(&frame_mutex);
...
#ifdef XZ_MODS_BRIDGE_V1
    __atomic_store_n(&vj_listener_ready, 1u, __ATOMIC_RELEASE);
#endif
...
#ifdef XZ_MODS_BRIDGE_V1
    __atomic_store_n(&vj_listener_ready, 0u, __ATOMIC_RELEASE);
#endif
    close(server);
    return NULL;
}
...
static void *vj_discovery_listener(void *unused) {
    ...
    __atomic_store_n(&vj_discovery_ready, 1u, __ATOMIC_RELEASE);
...
static int send_touch_event_checked(int client, uint8_t type, int x, int y, int value,
                                    int owned_release) {
    struct vjte_event event;
    if (client < 0 || client != vjfs_client_fd || x < 0 || y < 0) return 0;
    /* Only the native contact owner can finish an already-forwarded gesture
     * after the mod panel takes over. No new press gets this exemption. */
    if (owned_release) {
        if (type != 2) return 0;
    } else if (mods_panel_visible() || !overlay_is_active()) return 0;
    if (x > 65535) x = 65535;
    if (y > 65535) y = 65535;
...
static void send_touch_event(uint8_t type, int x, int y, int value) {
    (void)send_touch_event_checked(vjfs_client_fd, type, x, y, value, 0);
}

__attribute__((visibility("default")))
void xz_vj_touch_v1(int down, int x, int y) {
    static int native_client = -1;
    static int native_x, native_y;
    int pressed = down != 0;
    int valid_point = x >= 0 && x < 800 && y >= 0 && y < 480;
    if (pressed) {
        int client = vjfs_client_fd;
        if (native_client >= 0 || mods_panel_visible() || !valid_point) return;
        if (send_touch_event_checked(client, 1, x, y, 1, 0)) {
            native_client = client;
            native_x = x;
            native_y = y;
        }
    } else if (native_client >= 0) {
        int client = native_client;
        native_client = -1;
        (void)send_touch_event_checked(client, 2, valid_point ? x : native_x,
                                      valid_point ? y : native_y, 0, 1);
    }
}
...
static DFBResult hooked_input_get_event(IDirectFBEventBuffer *thiz, DFBEvent *event) {
    ...
    if (result == DFB_OK && event && event->clazz == DFEC_INPUT && !mods_owns_native_touch()) {
